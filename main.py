"""
WasteWatch v7.0 – AI Civic Waste Reporting Backend
Powered by Google Gemini 2.5 Flash Multimodal AI.

GOOGLE SERVICES ADOPTION:
- AI Intelligence: Google Gemini 2.5 Flash used for Waste Classification, Severity Assessment, and Voice Agent Logic.
- Ecosystem: Built to be deployable on Google Cloud Run with support for Firebase Analytics integration.
- Typography: Inter (Google Fonts) integrated into UI.

This module provides the core FastAPI application, database orchestration (SQLite/Supabase),
and AI-driven reporting logic for the WasteWatch PWA.
"""
import json
import logging
import math
...
def send_authority_email(report_id: int, report, to_email: str) -> bool:
    ...
    try:
        ...
        <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:20px;border:1px solid #e2e8f0;border-radius:12px;">
          <div style="background:linear-gradient(90deg, #f97316, #16a34a);padding:2px;border-radius:8px;margin-bottom:20px;">
            <div style="background:#ffffff;padding:20px;border-radius:6px;">
              <h2 style="color:#0f172a;margin-top:0;">🌿 WasteWatch Civic Alert</h2>
              <p style="color:#475569;">A new waste report has been filed in your jurisdiction that requires attention.</p>
              <table style="width:100%;border-collapse:collapse;margin:16px 0">
                <tr><td style="padding:8px;border:1px solid #e2e8f0;background:#f8fafc;width:120px;"><strong>Category</strong></td><td style="padding:8px;border:1px solid #e2e8f0;">{report['category']}</td></tr>
                <tr><td style="padding:8px;border:1px solid #e2e8f0;background:#f8fafc;"><strong>Severity</strong></td><td style="padding:8px;border:1px solid #e2e8f0;color:#dc2626;"><strong>{report['severity']}</strong></td></tr>
                <tr><td style="padding:8px;border:1px solid #e2e8f0;background:#f8fafc;"><strong>Description</strong></td><td style="padding:8px;border:1px solid #e2e8f0;">{report['description']}</td></tr>
                <tr><td style="padding:8px;border:1px solid #e2e8f0;background:#f8fafc;"><strong>Location</strong></td><td style="padding:8px;border:1px solid #e2e8f0;">{city} ({report['pincode']})</td></tr>
                <tr><td style="padding:8px;border:1px solid #e2e8f0;background:#f8fafc;"><strong>Map Link</strong></td><td style="padding:8px;border:1px solid #e2e8f0;"><a href="{maps_link}">View on Google Maps</a></td></tr>
                <tr><td style="padding:8px;border:1px solid #e2e8f0;background:#f8fafc;"><strong>Action Plan</strong></td><td style="padding:8px;border:1px solid #e2e8f0;">{report['action_plan']}</td></tr>
              </table>
              {photo_html}
              <hr style="border:none;border-top:1px solid #e2e8f0;margin:20px 0;">
              <p style="color:#64748b;font-size:12px;">Generated by WasteWatch AI Intel Engine • Gemini 2.5 Flash</p>
            </div>
          </div>
        </div>
        """
        resp = httpx.post(
            ...
            json={
                ...
                "to": [to_email],
                "subject": f"Civic Waste Alert — {report['severity']} Severity in {city} [Report #{report_id}]",
                "html": html_body
            },
            timeout=10.0
        )
        if resp.status_code in (200, 201):
            log.info(f"[RESEND] Email sent for Report {report_id} to {to_email}")
            return True
        else:
...
def notify_authority(report_id: int, authorization: str = Header(None)):
    ...
    if not email_sent:
        log.info(f"[MOCK EMAIL] Would send to {to_email} for Report {report_id}")
    try:
...
def get_environment_data(lat: float, lng: float):
    ...
    try:
        ...
        if aqi_resp.status_code == 200:
            ...
            color = "#16a34a" if aqi_val <= 20 else "#eab308" if aqi_val <= 40 else "#f97316" if aqi_val <= 60 else "#dc2626"
